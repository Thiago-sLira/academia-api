...
class DeletarFuncionarioIntegrationTest extends BaseIntegrationTest {

    private static final String URL_FUNCIONARIO_POR_ID = "/api/funcionarios/{id}";

    @Test
    @DisplayName("Deve deletar o funcionário com sucesso e remover apenas o registro correto do banco")
    @DataSet(value = "datasets/funcionario-existente.yml")
    @ExpectedDataSet(value = "datasets/funcionario-deletado-esperado.yml", ignoreCols = {"criado_em"})
    void deveDeletarFuncionarioComSucesso() throws Exception {
        mockMvc.perform(delete(URL_FUNCIONARIO_POR_ID, 1))
    ...
    void deveRetornar404QuandoIdNaoExistir() throws Exception {
        mockMvc.perform(delete(URL_FUNCIONARIO_POR_ID, 999))
    ...
    void deveRetornar400QuandoIdNaoForNumerico() throws Exception {
        mockMvc.perform(delete("/api/funcionarios/abc"))
